...
def get_opencl_devices():
    ...
    if cl_devices_avail is None:
        try:
            ...
            cl_devices_avail = [d for d in itertools.chain(*[p.get_devices() for p in pyopencl.get_platforms()]) if d.available==1 and d.profile=="FULL_PROFILE" and d.endian_little==1]
...
class WalletArmory(object):
    ...
    def _return_verified_password_or_false_opencl(self, passwords):
        ...
        v_len = self._kdf.getMemoryReqtBytes() // 64
        for i in range(self._kdf.getNumIterations()):
            ...
            for v_start in range(0, v_len - self._v_len_chunksize, self._v_len_chunksize):
                done = []  # a list of OpenCL event objects
            ...
            for v_start in range(0, v_len//2 - self._v_len_chunksize//2, self._v_len_chunksize//2):
                done = []  # a list of OpenCL event objects
                # Start up a kernel for each device to do one chunk of v_len_chunksize work
                for devnum in range(len(self._cl_devices)):
...
class WalletBitcoinCore(object):
    ...
    def _return_verified_password_or_false_opencl(self, passwords):
        ...
        if tstr == str:
            passwords = [p.encode("utf_8", "ignore") for p in passwords]
...
def expand_single_range(m):
    char_first, char_last = list(map(ord, m.groups()))
...
class MakePeekable(object):
    ...
    def __iter__(self):
        return self
    def __next__(self):
        return self.readline() if self._peeked else next(self._file)
    #
...
def open_or_use(filename, mode = "r",
    ...
    print(f"Attempting to open file: {filename}")  # 打印传入的路径jacky
    if not filename:  # 检查路径是否有效
        print("Filename is invalid, skipping file open.")
        return None
    try:
    ...
    print("filename:", filename)  # 打印传入的文件名jacky
   # print("kwds:", kwds)  # 打印传入的其他参数jacky
...
def init_parser_common():
    ...
    if not parser_common_initialized:
        ...
        for typo_name, typo_args in list(simple_typo_args.items()):
        ...
        for typo_name in itertools.chain(("swap",), list(simple_typo_args.keys()), ("insert",)):
...
def parse_arguments(effective_argv, wallet = None, base_iterator = None,
    ...
    for typo_name in itertools.chain(("swap",), list(simple_typos.keys()), ("insert",)):
    ...
    enabled_simple_typos = tuple(
        generator for name,generator in list(simple_typos.items()) if args.__dict__["typos_"+name])
    ...
    if enabled_simple_typos:
        max_simple_typos = \
            [args.__dict__["max_typos_"+name] for name in list(simple_typos.keys()) if args.__dict__["typos_"+name]]
    ...
    global passwordlist_file, initial_passwordlist, passwordlist_allcached
    print("args.passwordlist:", args.passwordlist) 
    print("kwds.get('passwordlist'):", kwds.get("passwordlist"))
    
    #passwordlist_file = open_or_use(args.passwordlist, "r", kwds.get("passwordlist"),
    #                                permit_stdin=True, decoding_errors="replace")
    passwordlist_file = open_or_use(args.passwordlist, "r", args.passwordlist,
                                    permit_stdin=True, decoding_errors="replace")
        ...
        for line_num, line in enumerate(map_file, 1):
            ...
            for c in split_line[0]:  # (c is the character to be replaced)
                ...
                if not same_permitted and c in replacements:
                    map_data[c] = [r for r in replacements if r != c]
                else:
...
def tokenlist_base_password_generator():
    ...
    for tokens_combination in product_generator:
        ...
        if not using_product_limitedlen:
            tokens_combination = [t for t in tokens_combination if t is not None]
    ...
    if not est_secs_per_password:
        ...
        # The still fairly simple case where there's not much to skip, just skip it all at once
            ...
            try:
                ...
                passwords_counted = next(passwords_count_iterator)
